...
// SearchKey inputs an LLIterator, a HTKey_t variable, and a HTKeyValue_t
// double pointer. This function traverses through all the nodes of a
// linkedlist pointed to by the iterator. If a traversed node contains a
// payload with a key equal to the HTKey_t variable that was passed it, it
// returns true and assigns the HTKeyValue_t double pointer variable to the
// address of the payload in the traversed node. Essentially, this function
// looks searches if a node with a payload containing a specific key exists
// in a linked list.
static bool SearchKey(LLIterator *lli, HTKey_t key, HTKeyValue_t** keyvalue);
...
bool HashTable_Insert(HashTable *table,
  ...
  HTKeyValue_t *the_payload;
  // Call SearchKey helper function to see if the key in newkeyvalue is already
  // contained in one of the nodes.
  if (SearchKey(lli, newkeyvalue.key, &the_payload)) {
    LLIterator_Free(lli);
    oldkeyvalue->key = the_payload->key;
    oldkeyvalue->value = the_payload->value;
    the_payload->value = newkeyvalue.value;
    return true;

  } else {
    HTKeyValue_t *new_payload = (HTKeyValue_t *) malloc(sizeof(HTKeyValue_t));
    new_payload->key = newkeyvalue.key;
    new_payload->value = newkeyvalue.value;
    LinkedList_Push(chain, (LLPayload_t) new_payload);
...
bool HashTable_Find(HashTable *table,
  ...
  // Call SearchKey helper function to see if the key in newkeyvalue is already
  // contained in one of the nodes.
  if (SearchKey(lli, key, &the_payload)) {
    keyvalue->key = the_payload->key;
    keyvalue->value = the_payload->value;
    LLIterator_Free(lli);
    return true;
  }
  LLIterator_Free(lli);
...
bool HashTable_Remove(HashTable *table,
  ...
  if (SearchKey(lli, key, &the_payload)) {
    keyvalue->key = the_payload->key;
    keyvalue->value = the_payload->value;
    LLIterator_Remove(lli, LLNoOpFree);
    table->num_elements -= 1;
    LLIterator_Free(lli);
    free(the_payload);
    return true;
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  int bucket_num = iter->bucket_idx;
  int num_buckets = iter->ht->num_buckets;
  HashTable *table = iter->ht;

  // If the current linked list does not have a next node, search for nodes in
  // the following linkedlist.
  for (int i = bucket_num+1; i<num_buckets; i++) {
    if (LinkedList_NumElements(table->buckets[i])) {
      LLIterator_Free(iter->bucket_it);
      iter->bucket_idx = i;
      iter->bucket_it = LLIterator_Allocate(table->buckets[i]);
      return true;
    }
  }
  return false;
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  if (!HTIterator_IsValid(iter) || !LLIterator_IsValid(iter->bucket_it)) {
    return false;
  }

  HTKeyValue_t *new_payload;
  LLIterator_Get(iter->bucket_it, (LLPayload_t) &new_payload);
  keyvalue->key = new_payload->key;
  keyvalue->value = new_payload->value;
...
static bool SearchKey(LLIterator *lli, HTKey_t key, HTKeyValue_t** keyvalue) {
  ...
  while (LLIterator_IsValid(lli)) {
    LLIterator_Get(lli, (LLPayload_t) &payload);
    if (payload->key == key) {
      *keyvalue = payload;
      return true;
    }
