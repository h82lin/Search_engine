...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  while (left_to_read > 0) {
    result = read(fd, buf + (file_size - left_to_read), left_to_read);
    if (result == -1) {
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  bool seen_boundary = true;
  bool seen_alphab = false;
  int position;
  int i = 0;
  while (1) {
    if (content[i] == '\0') {
    ...
    if (!isalpha(content[i])) {
      ...
      seen_boundary = true;
      if (seen_alphab) {
        AddWordPosition(tab, word_start, (DocPositionOffset_t) position);
        seen_alphab = false;
      }
    } else {
      content[i] = tolower(content[i]);
      if (seen_boundary == true) {
        word_start = content+i;
        position = i;
        seen_boundary = false;
        seen_alphab = true;
      }
